                    ...
                    if progress == 100 and locale not in downloaded:
                        ...
                        if self.locale_delimiter:
                            ...
                            if self.clone_option == 'on':
                                #this prevents recursion when clone option is turned on and a file in a subfolder is modified/added
                                clone_file_path = self.download.download_action(doc_id, locale, autoFormat, xliff=False, locale_ext=False)
                                #making paths a set prevents the list of paths to become full of duplicates from files added with the same name.
                                #note that this only triggers ltk watch if the file was created outside of the watch home dir first (ex. creating testing/test.txt then test.txt will trigger, creating test.txt then testing/test.txt will not)
